...
class MomentTopCell: UICollectionViewCell {
    fileprivate lazy var headIV: UIImageView = {
        let iv = UIImageView()
        iv.frame = CGRect(x: 0, y: 0, width: screenWidth, height: bounds.size.height - bottomIndent)
        iv.contentMode = .scaleAspectFill
        return iv
    }()
    fileprivate lazy var avatarIV: UIImageView = {
        let iv = UIImageView()
        iv.frame =  CGRect(x: screenWidth - avatorW - 16, y: headIV.frame.maxY, width: avatorW, height: avatorW)
