...
class CommentInputView: UIView {
    fileprivate(set) lazy var contentView: UIView = {
        let v = UIView()
        v.frame = CGRect(x: 0, y: screenHeight, width: screenWidth, height: contentMinHeight)
        v.backgroundColor = .groupTableViewBackground
        
        let layer = CALayer()
        layer.backgroundColor = UIColor.lightGray.cgColor
        layer.frame = CGRect(x: 0, y: 0, width: screenWidth, height: 0.5)
    ...
    fileprivate(set) lazy var textView: JXTextView = {
        ...
        tv.frame = CGRect(x: 15, y: surplusHeight/2, width: screenWidth-30, height: contentMinHeight-surplusHeight)
        return tv
    }()
...
fileprivate extension CommentInputView {
    ...
    func updateTop(rect: CGRect, duration: Double) {
        ...
        var top: CGFloat = 0
        if keyboardH > 0 {
            top = screenHeight - contentView.frame.height - keyboardHeight
        } else {
            top = screenHeight
        }
        if ctTop == top { return }
    ...
    func updateHeight(_ height: CGFloat) {
        ...
        ctTop = screenHeight - ctHeight - keyboardHeight
