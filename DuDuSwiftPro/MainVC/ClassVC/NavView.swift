...
class NavView: UIView {
    ...
    lazy var navlb: UIView = {
        ...
        lb.frame = CGRect(x: 0, y: navView.frame.height - 40, width: screenWidth, height: 30)
        return lb
    }()
    ...
    lazy var camareBtn: UIButton = {
        ...
        btn.frame = CGRect(x: screenWidth - 20 - 30, y: navView.frame.height - 40, width: 30, height: 30)
        btn.addTarget(self, action: #selector(click(_:)), for: .touchUpInside)
        btn.tag = 1
        return btn
    }()
